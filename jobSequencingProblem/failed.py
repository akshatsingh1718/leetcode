...
class Solution:
    ...
    def JobScheduling(self, Jobs: List[Job], n):
        ...
        while i < n:
            ...
            curr_deadline = Jobs[i].deadline
            jobs_done += 1

            while i + 1 < n and curr_deadline == Jobs[i + 1].deadline:
                curr_profit = max(curr_profit, Jobs[i + 1].profit)
                i += 1
...
class Solution:
    ...
    def JobScheduling(self, Jobs: List[Job], n):
        ...
        time_left = max(Jobs, key=lambda x: x.deadline).deadline
