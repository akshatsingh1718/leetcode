...
class Solution:
    ...
    SC: O(n^2) [new grid if not inplace] + O(n) [deque]

    visited set removed as we can use ids for checking visited or not
    ...
    def largestIsland(self, grid: List[List[int]]) -> int:
        ...
        def bfs(i: int, j: int, island_id: int) -> int:
        ...
        for i in range(ROWS):
            for j in range(COLS):
                if grid[i][j] == 1:
                    ...
                    grid[i][j] = curr_id
                    id_to_area[curr_id] = bfs(i, j, curr_id)
