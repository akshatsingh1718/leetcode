...
class Solution:
    ...
    n = len(tree_nodes)
    TC: O(n) [dfs]
    SC: O(n) [traversal for arr recursion stack]

    ==========================
    Algorithm: (dfs)
    ==========================
    1. create a global sum which will accumulate the sums from gt values of the current node.
    2. Run a dfs traversal and go straight to right then update root.val and then go left.
    3. Running sum will take all the sums from the greater values.
    """
