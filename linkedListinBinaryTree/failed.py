...
class Solution:
    ...
    def isSubPath(self, head: Optional[ListNode], root: Optional[TreeNode]) -> bool:
        ...
        queue = [
            [
                (root, head if head.val != root.val else head.next),
            ]
        ]

        while queue:

            for node, ll_node in queue.pop(0):
                ...
                if node.left is not None:
                    nxt_node = head
                    if ll_node.val == node.left.val:
                        nxt_node = ll_node.next
                    elif head.val == node.left.val:
                        nxt_node = head.next
                    tqueue.append(
                        (
                            node.left,
                ...
                if node.right is not None:
                    nxt_node = head
                    if ll_node.val == node.right.val:
                        nxt_node = ll_node.next
                    elif head.val == node.right.val:
                        nxt_node = head.next
                    tqueue.append((node.right, nxt_node))
                    if nxt_node is None:
                        return True
                if len(tqueue) > 0:
                    queue.append(tqueue)
