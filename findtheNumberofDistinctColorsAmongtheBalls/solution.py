...
class Solution:
    ...
    def queryResults(self, limit: int, queries: List[List[int]]) -> List[int]:
        ...
        color_to_idx = defaultdict(set)
        res = []

        for idx, color in queries:

            if balls_color[idx] == 0:  # first time color change
                color_to_idx[color].add(idx)
                ...
                # remove the idx from old color
                color_to_idx[old_color].remove(idx)
                # check if the set is empty now
                if len(color_to_idx[old_color]) == 0:
                    del color_to_idx[old_color]

            res.append(len(color_to_idx))
